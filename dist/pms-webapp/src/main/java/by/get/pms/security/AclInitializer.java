...
public class AclInitializer implements InitializingBean {
    ...
    public AclInitializer(DataSource dataSource, PlatformTransactionManager transactionManager, MutableAclService mutableAclService) {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        Authentication authRequest = new UsernamePasswordAuthenticationToken("bond",
                "007", AuthorityUtils.createAuthorityList("ROLE_IGNORED"));
    ...
    private void createProjectACLs(long[] projectIds) {
        for (long projectId : projectIds) {
            final ObjectIdentity projectObjectIdentity = new ObjectIdentityImpl(Project.class,
                    new Long(projectId));
    ...
    private void populateProjectACLs(Map<Long, String> projectIdToAssignedPMUsername, Map<Long, List<String>> projectIdsToAllocatedUsers) {
    ...
    private void populateProjectACLsForAdmin(Set<Long> projectIds) {
        projectIds.forEach(projectId -> {
            final ObjectIdentity projectObjectIdentity = new ObjectIdentityImpl(Project.class,
                    new Long(projectId));
            grantPermissionToObjectIdentityForRole(projectObjectIdentity, UserRole.ROLE_ADMIN, BasePermission.ADMINISTRATION);
        });
    }
    ...
    private void populateProjectACLsForPMs(Map<Long, String> projectIdsToAssignedPMUsernames) {
        for (Map.Entry<Long, String> entry : projectIdsToAssignedPMUsernames.entrySet()) {
            ...
            grantPermissionToObjectIdentityForPrincipal(projectObjectIdentity, entry.getValue(), BasePermission.ADMINISTRATION);
        }
    }

    private void grantPermissionToObjectIdentityForPrincipal(ObjectIdentity objectIdentity, String recipientUsername, Permission permission) {
        AclImpl acl = (AclImpl) mutableAclService.readAclById(objectIdentity);
    ...
    private void populateProjectACLsForUsers(Map<Long, List<String>> projectIdsToAllocatedUsers) {
        for (Map.Entry<Long, List<String>> longListEntry : projectIdsToAllocatedUsers.entrySet()) {
            ...
            for (String recipientUsername : longListEntry.getValue()) {
                grantPermissionToObjectIdentityForPrincipal(projectObjectIdentity, recipientUsername, BasePermission.READ);
            }
        }
    }
    ...
    private void createTaskACLs(long[] taskIds) {
        for (long taskId : taskIds) {
            final ObjectIdentity taskObjectIdentity = new ObjectIdentityImpl(Task.class,
                    new Long(taskId));
    ...
    private void populateTaskACLsForAdmin(Set<Long> taskIds) {
        taskIds.forEach(taskId -> {
            final ObjectIdentity taskObjectIdentity = new ObjectIdentityImpl(Task.class,
                    new Long(taskId));
            grantPermissionToObjectIdentityForRole(taskObjectIdentity, UserRole.ROLE_ADMIN, BasePermission.ADMINISTRATION);
        });
    }
    ...
    private void populateTaskACLsForUsers(Map<Long, String> taskIdsToAssignedUsernames) {
        for (Map.Entry<Long, String> longStringEntry : taskIdsToAssignedUsernames.entrySet()) {
            ...
            grantPermissionToObjectIdentityForPrincipal(taskObjectIdentity, longStringEntry.getValue(), BasePermission.ADMINISTRATION);
        }
    }
