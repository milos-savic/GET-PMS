...
class TaskServiceImpl implements TaskService {
	...
	public List<TaskDTO> getAll() {
		...
		return tasks.parallelStream().map(Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER::apply)
	...
	public TaskDTO getTask(Long taskId) {
		return Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER.apply(taskDao.findOne(taskId));
	}
	...
	public TaskDTO getTaskByProjectAndName(ProjectDTO projectDTO, String name) {
		...
		return Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER
				.apply(taskDao.findTaskByProjectAndName(projectData, name));
	}
	...
	public List<TaskDTO> getProjectTasks(ProjectDTO projectDTO) {
		...
		return projectTasks.parallelStream().map(Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER::apply)
				.collect(Collectors.toList());
	}
	...
	public List<TaskDTO> getTasksAssignedToUser(UserDTO userDTO) {
		...
		return tasksAssignedToUser.parallelStream().map(Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER::apply)
				.collect(Collectors.toList());
	}
	...
	public TaskDTO createTask(TaskDTO taskParams) {
		...
		return Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER.apply(taskData);
	}
	...
	public void updateTaskByProjectManager(TaskUpdateParamsForPM taskUpdateParamsForPM) {
		TaskData taskDataFromDb = taskDao.findOne(taskUpdateParamsForPM.getId());
		BeanUtils.copyProperties(taskUpdateParamsForPM, taskDataFromDb);

		updateTask(Data2DtoTransformers.TASK_DATA_2_TASK_DTO_TRANSORMER.apply(taskDataFromDb));
	}

	@Override
	@Transactional
	public void updateTaskByDeveloper(TaskUpdateParamsForDev taskUpdateParamsForDev) {
		TaskData taskDataFromDb = taskDao.findOne(taskUpdateParamsForDev.getId());
		BeanUtils.copyProperties(taskUpdateParamsForDev, taskDataFromDb);
